...
def _fetch_kline_entries(symbol: str, timeframe: str, limit: int = 1500) -> list:
    ...
    if PT_DEBUG:
        ...
        print(
            f"[PT_DEBUG] Trainer candles symbol={symbol} tf={timeframe} "
            f"count={len(candles)} first_ts={first_ts} last_ts={last_ts}"
        )
        if not candles:
            raise RuntimeError(
                f"Trainer candle preflight failed for {symbol} timeframe={timeframe}."
            )
    entries = []
    for c in candles:
...
def get_memory_path(tf_choice: str, kind: str) -> str:
    safe_tf = str(tf_choice or "").strip()
    filename = f"{kind}_{safe_tf}.txt"
...
def load_memory(tf_choice):
    ...
    data["memory_list"] = _read_text(get_memory_path(tf_choice, "memories")).replace("'","").replace(',','').replace('"','').replace(']','').replace('[','').split('~')
    data["weight_list"] = _read_text(get_memory_path(tf_choice, "memory_weights")).replace("'","").replace(',','').replace('"','').replace(']','').replace('[','').split(' ')
    data["high_weight_list"] = _read_text(get_memory_path(tf_choice, "memory_weights_high")).replace("'","").replace(',','').replace('"','').replace(']','').replace('[','').split(' ')
    data["low_weight_list"] = _read_text(get_memory_path(tf_choice, "memory_weights_low")).replace("'","").replace(',','').replace('"','').replace(']','').replace('[','').split(' ')
...
def flush_memory(tf_choice, force=False):
    ...
    if (not data.get("dirty")) and (not force):
        return
    try:
        with open(get_memory_path(tf_choice, "memories"), "w+", encoding="utf-8") as f:
            f.write("~".join([x for x in data["memory_list"] if str(x).strip() != ""]))
    except:
        pass
    try:
        with open(get_memory_path(tf_choice, "memory_weights"), "w+", encoding="utf-8") as f:
            f.write(" ".join([str(x) for x in data["weight_list"] if str(x).strip() != ""]))
    except:
        pass
    try:
        with open(get_memory_path(tf_choice, "memory_weights_high"), "w+", encoding="utf-8") as f:
            f.write(" ".join([str(x) for x in data["high_weight_list"] if str(x).strip() != ""]))
    except:
        pass
    try:
        with open(get_memory_path(tf_choice, "memory_weights_low"), "w+", encoding="utf-8") as f:
...
def write_threshold_sometimes(tf_choice, perfect_threshold, loop_i, every=200):
    ...
    if (loop_i % every != 0) and (last is not None) and (abs(perfect_threshold - last) < 0.05):
        return
    try:
        with open(get_memory_path(tf_choice, "neural_perfect_threshold"), "w+", encoding="utf-8") as f:
...
def _ensure_memory_files() -> None:
    for tf in ["2hour", "4hour", "8hour", "12hour", "1day", "1week"]:
        load_memory(tf)
                    ...
                    try:
                        ...
                        if 1 == 1:
                            try:
                                ...
                                memory_list = mem_data.get("memory_list", [])
                                weight_list = mem_data.get("weight_list", [])
                                high_weight_list = mem_data.get("high_weight_list", [])
                                low_weight_list = mem_data.get("low_weight_list", [])
                                mem_ind = 0
                                diffs_list = []
                                any_perfect = 'no'
                                perfect_dexs = []
                                                ...
                                                while True:
                                                    ...
                                                    try:
                                                        ...
                                                        try:
                                                            ...
                                                            while True:
                                                                new_memory = 'no'
                                                                if (not moves) or (indy < 0) or (indy >= len(moves)):
                                                                    if PT_DEBUG and (not _moves_warned):
                                                                        _moves_warned = True
                                                                        print(f"[PT_DEBUG] Moves index out of range: indy={indy} len(moves)={len(moves)}")
                                                                    break
                                                                var3 = (moves[indy]*100)
